...
impl Downloader {
  // pub fn new<S, P>(urls: Vec<S>, target_dir: P, config: Config) ->
  // Result<Self> where
  //   S: AsRef<str>,
  //   P: AsRef<Path>
  // {
  ...
  //   let client = client_builder.build().map_err(|e| Error::Configuration {
  //     message: format!("Failed to create HTTP client: {e}",)
  //   })?;

  //   let validator = validation::UrlValidator::new(&config);
  //   let event_sink = config.event_sink.clone();

  //   info!(
  //     "Initiated {} for {url_count} files from {urls:#?} to {target_dir:#?}",
  //     env!("CARGO_PKG_NAME")
  //   );
  //   debug!("{config:#?}");
  ...
  ///
  /// This is a convenience method that uses sensible defaults for most use
  /// cases.
