...
impl Downloader {
  ...
  pub fn new<S: AsRef<str>, P: AsRef<Path>>(
    ...
    let downloader = Self {
      ...
      home,
      concurrency_limit,
      already_exists: None, // Correctly indicates "not checked yet"
    };
    trace!("Initiated {downloader:#?}");
    downloader
  }
  ...
  pub async fn start(&mut self, force: bool) -> Result<()> {
    ...
    for (index, url_str) in self.urls.iter().enumerate() {
      Url::parse(url_str)
        ...
        .map(|_| trace!("URL {} validated: {}", index, url_str))?;
    }
    ...
    for (index, url_str) in self.urls.iter().enumerate() {
      ...
      trace!("Task {}: {} -> {:?}", index, url_str, final_path);
    ...
    for (index, result) in results.into_iter().enumerate() {
      match result {
        Ok(_) => {
          success_count += 1;
          trace!("Download {} completed successfully", index);
        }
        Err(e) => {
          trace!("Download {} failed: {}", index, e);
          return Err(e);
        }
      }
    }

    trace!("All {} downloads completed successfully", success_count);
    Ok(())
  }
